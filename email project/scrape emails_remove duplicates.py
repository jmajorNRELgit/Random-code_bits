...
def check_list(lis):
    ...
    for j in range(len(lis)): # j holds one address at a time
        for i in range(len(lis)): #compares the rest to j
            if i != j and i not in dup_list: #prevents catching the same duplicate twice
...
duplicate_locations = pd.DataFrame(check_list(combined_emails)) #turn data into a Dataframe to save as csv
