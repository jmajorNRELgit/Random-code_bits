...
def max_torque():
    ...
    color = list(map_image[79][121]) #max torque line color. this can reference an existing pixel or be a list of known RGB values
    torque_max_pix = [] #variable for the Y-axis location of the pixel
    speed_pix = [] #variable for the X-axis location of the pixel
    for j in range(120,1505): #range is the number of columns in the image map area (area of interest) 119,1505
        for i in range(780,79,-1): #range is the number of rows. Starts at the bottom and moves up
            pix = map_image[i][j]
            if list(pix) == color:
...
torque_max_pix = [x + 4 for x in torque_max_pix]
