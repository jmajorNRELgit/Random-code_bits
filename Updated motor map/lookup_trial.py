...
def setup():
    ...
    percent_file = 'leaf inverter bar.PNG'
    ...
    percent_bar_x = [880,581,282,14] #this is the corrosponding pixel location
    ...
    '''Change these'''
    for i in range(14, 880): #the range is the pixel locations within the color bar itself (no white or black...)
...
torque_range = np.arange(0, 278) #the max torque value

speed_range = np.arange(495,10000,5) #the RPM values from the Max torque list
...
start = time.time()
for i in range(0,1901): #this is the number of columns in the lookup_table (or however many you want to try)
    print(i)
    for j in range(0,278): #this is the number of rows in the lookup table
