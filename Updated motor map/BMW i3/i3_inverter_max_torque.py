...
def max_torque():
    ...
    color = list(map_image[25][115]) #max torque line color. this can reference an existing pixel or be a list of known RGB values
    torque_max_pix = [] #variable for the Y-axis location of the pixel
    speed_pix = [] #variable for the X-axis location of the pixel
    for j in range(113,1525): #range is the number of columns in the image map area (area of interest) 119,1505
        for i in range(700,22,-1): #range is the number of rows. Starts at the bottom and moves up
            pix = map_image[i][j]
            if list(pix) == color:
...
def setup():
    ...
    plot the image using plt.imshow and look at the x and y scale at the image tic marks'''
