...
def setup():
    ...
    '''Change these'''
    for i in range(12, 864): #the range is the pixel locations within the color bar itself (no white or black...)
...
#these next few lines set up the pandas dataframe
torque_range = np.arange(0, 247) #the max torque value
speed_range = np.arange(526,11466,5) #the RPM values from the Max torque list
...
start = time.time()
for i in range(0,2188): #this is the number of columns in the lookup_table (or however many you want to try)
    print(i)
    for j in range(0,247): #this is the number of rows in the lookup table
